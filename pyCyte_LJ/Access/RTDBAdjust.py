...
#            self.des['conc'].iloc[c] = self.des['concentration'].iloc[c].split('__')[0].replace(' ','')
            self.des['Conc'].iloc[c] = str(self.des['Conc'].iloc[c])
...
#                        self.db[self.var].iloc[i] = y
    ...
    def updateDB(self, GUI = True,plateType = 'None', parameter = 'None'):
        if GUI == True:
            self.plateType=self.platevar.get()
            self.parameter=self.tablevar.get()
        elif GUI == False:
        ...
        if self.parameter == 'CF':
            ...
            try:
                ...
                print('Unable to write log. File is open. Cannot proceed.')
            return

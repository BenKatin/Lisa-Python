                     ...
                     if (fluid == 'MPD'):
                         ...
                         if (version == '1' ):
                             ...
                             if ( fluid == 'DMSO' ):
                                 ...
                                 else:   ## Adding version 3 for PPL_GPSA and GPSB on 5/30/18 LJ
                                         ## Major hole: I want this fill pattern for PPL_Plus_AQ_GP, 
                                         ##but not other plates with Fluid = Gly.  I don't know how to split that log,
                                         ## so run this code only to make only the Plus_AQ_GP/GPSA/GPSB FT  plates.
                                         ## The Plus_AQ BP and SP use the second map.
                                 
                                     if (fluid == 'GPS14' ) or (fluid == 'GPS25') or (fluid == 'GLY'):
#                                         version = '2'
                                         thismap = populatePPFTVolumes(thismap, nrows, ncols, [20,30,40,65], version)
                                     else:
...
def genLDVPlusGPMaps():
    ...
    vol=14
    style='DD'    
    m = genMap(wells=wells,geo=geo,style=style,fluid=fluid,version=version,volume=vol)
    thisentry = writeMap(m,platetype=platetype,nwells=wells, geo=geo, style=style,fluid=fluid,version=version,vol=vol)
    allmaps.append(thisentry)    
    return allmaps   
...
def genPPPlusGPMaps():
    ...
    for i, platetype in enumerate(platetypes):    
        ...
        for c in concentrations:
            ...
            thisentry = writeMap(m,platetype=platetype, nwells=nwells, geo=geo, style=style, fluid=fluid, conc=c,version='2')
            allmaps.append(thisentry)
        
        style='DD'
        vol=60
        fluid += '-DF'
        m = genMap(wells=nwells,geo=geo,style=style,fluid=fluid,conc='-',version=version,volume=vol)
        thisentry = writeMap(m,platetype=platetype, nwells=nwells, geo=geo, style=style, fluid=fluid, conc='-',version=version, vol=vol) 
        allmaps.append(thisentry)
...
def genPPSPBPPlusMaps():
    ...
    for i, platetype in enumerate(platetypes):
        ...
        style = 'FF'
        for v in vols:
            m = genMap(wells=nwells,geo=geo,style=style,fluid=fluid,conc=100,impedance=impedance,version=version, volume=v)
        ...
        vol=60   
        fluid += '-DF'
        m = genMap(wells=nwells,geo=geo,style=style,fluid=fluid,conc=100,impedance=impedance, version=version, volume=vol)
        thisentry = writeMap(m,platetype=platetype, nwells=nwells, geo=geo, style=style, fluid=fluid, conc=100,vol=vol,version=version)
        allmaps.append(thisentry)
    return allmaps
...
def gen525SPMaps():
    ...
    for i in range(0,len(platetypes)):
       ...
       # overwrite 15
        ...
        vol=60 
        fluid += '-DF'
        m = genMap(wells=nwells,geo=geo,style=style,fluid=fluid,conc=concentration,impedance=impedance,version=version, volume=vol)
        thisentry = writeMap(m,platetype=platetype, nwells=nwells, geo=geo, style=style, fluid=fluid, conc=100,vol=vol,version=version)
        allmaps.append(thisentry)
...
def gen525BPMaps():
    ...
    for platetype in platetypes:
        ...
        vol=60   
        fluid = 'PBS-DF'
        m = genMap(wells=nwells,geo=geo,style=style,fluid=fluid,conc=100,impedance = 1.48,version=version, volume=vol)
        thisentry = writeMap(m,platetype=platetype, nwells=nwells, geo=geo, style=style, fluid=fluid, conc=100,vol=vol,version=version)
        allmaps.append(thisentry)
    return allmaps
